...
| **M5** | Image drop | ✅ **Done** — drag/drop + paste with thumbnails, user-bubble previews, OpenAI vision content-parts on the wire (text-only messages stay plain strings), conservative image token costing in Pack. Gating is optimistic: attach always allowed, a non-vision endpoint's error surfaces in the transcript. | ~2–3 days |
| **M6** | Packaging | ⏭ **Next** — electron-builder installers (Win/macOS), code signing, auto-update, binary bundling/resolution (bundle a pinned `nogreenteagc` build + settings override), app icon. Also pending: multi-workspace tabs, log-event surfacing, vision-model live test of image drop. | ~1 week |
