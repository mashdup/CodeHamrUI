...
| **P6** | Signing + auto-update | ✅ **Wired** — electron-updater feeds off GitHub Releases (background download, "Update — restart" banner, never auto-restarts; verified `app-update.yml` + `latest.yml` in the packaged build). macOS signing + notarization run in CI as soon as the 5 Apple secrets are added (see README); until then mac builds stay unsigned. **Windows unsigned by choice** — revisit if distribution widens. | ✅ done |
